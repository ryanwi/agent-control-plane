...
# Type alias for the session_scope callable passed to gateways
_SessionScope = Callable[[], AbstractAsyncContextManager[AsyncSession]]
_UowFactory = Callable[[AsyncSession], AsyncSqlAlchemyUnitOfWork]


class _AsyncGatewayBase:
    """Shared helpers for async gateways."""

    def __init__(self, session_scope: _SessionScope, uow_factory: _UowFactory) -> None:
    ...
    async def _record(
        ...
        *,
        ...
        if command_id is None:
            return
        await uow.command_repo.record_command(str(command_id), operation, result, session_id=session_id)
    ...
    async def _replay_events(self, session_id: UUID, *, limit: int = 10_000) -> list[EventFrame]:
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            return await uow.event_repo.replay(session_id, after_seq=0, limit=limit)
    ...
    async def _get_pending_tickets(self, session_id: UUID | None = None) -> list[ApprovalTicket]:
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            return await uow.approval_repo.get_pending_tickets(session_id)


class AsyncSessionGateway(_AsyncGatewayBase):
    ...
    def __init__(
        ...
        super().__init__(session_scope, uow_factory)
        self._mapper = mapper
    ...
    async def open_session(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_OPEN_SESSION)
            ...
            await self._record(uow, command_id, CMD_OPEN_SESSION, {"session_id": str(cs.id)}, session_id=cs.id)
            await uow.commit()
            return cs.id
    ...
    async def close_session(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_CLOSE_SESSION)
            if cached is not None:
            ...
            await uow.session_repo.update_session(
                session_id, status=SessionStatus.COMPLETED, active_cycle_id=None, updated_at=datetime.now(UTC)
            )
            await uow.commit()
            ...
            await self._record(
                uow, command_id, CMD_CLOSE_SESSION, result.model_dump(mode="json"), session_id=session_id
            )
            return result
    ...
    async def abort_session(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_ABORT_SESSION)
            if cached is not None:
                return SessionLifecycleResult.model_validate(cached)
            await uow.session_repo.update_session(
                session_id,
            ...
            await self._record(
                uow, command_id, CMD_ABORT_SESSION, result.model_dump(mode="json"), session_id=session_id
            )
            return result
    ...
    async def emit(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, meta.command_id, CMD_EMIT)
            if cached is not None:
                seq = cached.get("seq")
            ...
            seq = await uow.event_repo.append(
                ...
                payload=dict(payload),
                state_bearing=state_bearing,
                metadata=meta.as_event_metadata(),
            )
            await self._record(uow, meta.command_id, CMD_EMIT, {"seq": seq}, session_id=session_id)
            await uow.commit()
            return seq
    ...
    async def kill_session(
        ...
        *,
        ...
        return await self._trigger_kill(
            KillSwitchScope.SESSION_ABORT, session_id=session_id, reason=reason, command_id=command_id
        )

    async def kill_system(self, *, reason: str = "System halt", command_id: IdempotencyKey | None = None) -> KillResult:
    ...
    async def _trigger_kill(
        ...
        *,
        ...
        async with self._session_scope() as db:
            ...
            cached = await self._cached(uow, command_id, operation)
            if cached is not None:
            ...
            if scope == KillSwitchScope.SESSION_ABORT:
                ...
                result = KillResult(scope=KillSwitchScope.SESSION_ABORT, session_id=session_id, tickets_denied=denied)
                await self._record(uow, command_id, operation, result.model_dump(mode="json"), session_id=session_id)
                await uow.commit()
                return result
            if scope == KillSwitchScope.SYSTEM_HALT:
                ...
                result = KillResult(
                    scope=KillSwitchScope.SYSTEM_HALT, sessions_aborted=len(sessions), tickets_denied=denied_total
                )
                await self._record(uow, command_id, operation, result.model_dump(mode="json"))
                await uow.commit()
                return result
            raise ValueError(f"Unsupported kill scope: {scope}")


class AsyncLifecycleGateway(_AsyncGatewayBase):
    """Async-specific session state transitions and cycle management."""
...
class AsyncApprovalGateway(_AsyncGatewayBase):
    """Approval tickets, action proposals, and expiry."""
    ...
    async def create_ticket(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_CREATE_TICKET)
            if cached is not None:
            ...
            await self._record(
                uow, command_id, CMD_CREATE_TICKET, ticket.model_dump(mode="json"), session_id=session_id
            )
            await uow.commit()
            return ticket
    ...
    async def create_proposal(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_CREATE_PROPOSAL)
            if cached is not None:
    ...
    async def approve_ticket(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_APPROVE_TICKET)
            if cached is not None:
                return ApprovalTicket.model_validate(cached)
            ...
            await self._record(
                uow, command_id, CMD_APPROVE_TICKET, result.model_dump(mode="json"), session_id=ticket.session_id
            )
            await uow.commit()
            return result
    ...
    async def deny_ticket(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            cached = await self._cached(uow, command_id, CMD_DENY_TICKET)
            if cached is not None:
                return ApprovalTicket.model_validate(cached)
            ticket = await uow.approval_repo.get_pending_ticket_for_update(ticket_id)
            fields: ApprovalTicketUpdateFields = {
    ...
    async def get_pending_tickets(self, session_id: UUID | None = None) -> list[ApprovalTicket]:
        return await self._get_pending_tickets(session_id)
...
class AsyncBudgetGateway(_AsyncGatewayBase):
...
class AsyncAgenticGateway(_AsyncGatewayBase):
    ...
    async def create_goal(
        ...
        *,
        ...
        await self._emit_event(session_id, EventKind.GOAL_CREATED, goal.model_dump(mode="json"), state_bearing=True)
        return goal
    ...
    async def create_plan(self, session_id: UUID, goal_id: UUID, *, title: str, steps: list[str]) -> Plan:
        ...
        await self._emit_event(session_id, EventKind.PLAN_CREATED, plan.model_dump(mode="json"), state_bearing=True)
        return plan
    ...
    async def start_plan_step(self, session_id: UUID, plan_id: UUID, *, step_index: int) -> PlanStep:
        ...
        await self._emit_event(
            session_id, EventKind.PLAN_STEP_STARTED, step.model_dump(mode="json"), state_bearing=True
        )
        return step
    ...
    async def complete_plan_step(
        ...
        await self._emit_event(
            session_id, EventKind.PLAN_STEP_COMPLETED, step.model_dump(mode="json"), state_bearing=True
        )
        return step
    ...
    async def record_evaluation(
        ...
        *,
        ...
        await self._emit_event(session_id, event_kind, result.model_dump(mode="json"), state_bearing=False)
        return result
    ...
    async def apply_guardrail(
        ...
        *,
        ...
        await self._emit_event(
            session_id, guardrail_event_kind(phase), result.model_dump(mode="json"), state_bearing=False
        )
        return result
    ...
    async def create_checkpoint(
        ...
        *,
        ...
        async with self._session_scope() as db:
            ...
            await self._record(uow, command_id, operation, cp.model_dump(mode="json"), session_id=session_id)
            await uow.commit()
            return cp

    async def list_checkpoints(self, session_id: UUID, *, limit: int = 50, offset: int = 0) -> Page[SessionCheckpoint]:
        rows = await self._replay_events(session_id)
        checkpoints = [
...
class AsyncControlPlaneObserver(_AsyncGatewayBase):
    """Read-only queries, health checks, and operational metrics."""
    ...
    async def list_sessions(
        ...
        *,
        ...
        return await self._list_sessions(statuses=statuses, limit=limit)
    ...
    async def get_state_change_feed(
        ...
        *,
        ...
        async with self._session_scope() as db:
            uow = self._uow_factory(db)
            rows = await uow.event_repo.list_state_bearing_events(session_id=session_id, offset=cursor, limit=limit + 1)
            has_more = len(rows) > limit
    ...
    async def get_health_snapshot(self) -> SessionHealth:
        created = await self._list_sessions(statuses=[SessionStatus.CREATED], limit=10_000)
        active = await self._list_sessions(statuses=[SessionStatus.ACTIVE], limit=10_000)
        paused = await self._list_sessions(statuses=[SessionStatus.PAUSED], limit=10_000)
        pending = await self._get_pending_tickets()
    ...
    async def get_operational_scorecard(
        ...
        *,
        ...
        all_sessions = (
            [session_id] if session_id is not None else [s.id for s in await self._list_sessions(limit=10_000)]
        )
        ...
        for sid in all_sessions:
            events = await self._replay_events(sid)
            for event in events:
...
class AsyncMaintenanceGateway(_AsyncGatewayBase):
    """Background maintenance: stuck cycle recovery and timeout checking."""

    async def recover_stuck_sessions(self) -> dict[str, int]:
        sessions = await self._list_sessions(statuses=[SessionStatus.ACTIVE], limit=1000)
        stuck_sessions = recovered = aborted = 0
        for cs in sessions:
            ...
            async with self._session_scope() as db:
                uow = self._uow_factory(db)
                try:
                    await uow.session_repo.update_session(cs.id, active_cycle_id=None, updated_at=datetime.now(UTC))
                    await uow.event_repo.append(
                        cs.id,
            ...
            async with self._session_scope() as db:
                ...
                if last_event is None or last_event.created_at < cutoff:
                    ...
                    await uow.event_repo.append(
                        cs.id,
                        EventKind.KILL_SWITCH_TRIGGERED,
                        {"scope": "cycle_timeout", "timeout_seconds": timeout_seconds},
                        state_bearing=True,
                    )
                    escalated += 1
                await uow.commit()
...
class AsyncControlPlaneFacade:
    ...
    def __init__(
        ...
        *,
        ...
        self.sessions: AsyncSessionGateway = AsyncSessionGateway(
            self.session_scope, self._uow_factory, mapper, unknown_policy
        )
        self.lifecycle: AsyncLifecycleGateway = AsyncLifecycleGateway(self.session_scope, self._uow_factory)
        self.approvals: AsyncApprovalGateway = AsyncApprovalGateway(self.session_scope, self._uow_factory)
        self.budget: AsyncBudgetGateway = AsyncBudgetGateway(self.session_scope, self._uow_factory)
    ...
    async def session_scope(self) -> AsyncIterator[AsyncSession]:
        if self._engine is not None and not self._schema.initialized:
            async with self._schema.lock:
                if not self._schema.initialized:
    ...
    async def token_budget_tracker(self) -> AsyncIterator[Any]:
        ...
        ledger row persists even when the budget is exceeded. Other exceptions
