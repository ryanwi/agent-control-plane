...
def _ensure_reference_models_registered() -> None:
    ...
    consumers using a repo directly (CLI tools, admin scripts, custom paths)
...
class AsyncSqlAlchemyApprovalRepo:
    ...
    async def get_ticket_for_update(self, ticket_id: UUID) -> ApprovalTicket:
        approval_ticket_model = ModelRegistry.get("ApprovalTicket")
        result = await self._session.execute(
            select(approval_ticket_model).where(approval_ticket_model.id == ticket_id).with_for_update()
        )
...
class AsyncSqlAlchemyProposalRepo:
    ...
    async def get_proposal(self, proposal_id: UUID) -> ActionProposal | None:
        action_proposal_model = ModelRegistry.get("ActionProposal")
        result = await self._session.execute(
            select(action_proposal_model).where(action_proposal_model.id == proposal_id)
        )
        row = result.scalar_one_or_none()
        return self._to_dto(row) if row else None
...
class AsyncSqlAlchemyCommandRepo:
    """Async SQLAlchemy implementation of AsyncCommandRepository."""

    def __init__(self, session: AsyncSession) -> None:
        self._session = session

    async def get_command(self, command_id: str) -> CommandResult | None:
...
class AsyncSqlAlchemyTokenBudgetRepo:
    """Async SQLAlchemy implementation of AsyncTokenBudgetRepository."""
    ...
    async def get_budget_config(self, config_id: UUID) -> TokenBudgetConfig | None:
        ...
        result = await self._session.execute(select(model).where(model.id == config_id))
        row = result.scalar_one_or_none()
    ...
    async def list_budget_configs(self, identity: IdentityContext) -> list[TokenBudgetConfig]:
        model = ModelRegistry.get("TokenBudgetConfig")
        query = select(model)
        # Build identity subset matching: config matches if its non-None fields match
        conditions = []
    ...
    async def get_usage_summary(
        ...
        breakdown_result = await self._session.execute(breakdown_query)
...
class AsyncSqlAlchemyUnitOfWork:
    ...
    def __init__(self, session: AsyncSession) -> None:
        ...
        self.command_repo = AsyncSqlAlchemyCommandRepo(session)
