...
class ActionName(StrEnum):
    """Sentinel for unclassified actions. Use plain strings for domain action names."""
...
def parse_action_name(value: ActionValue) -> ActionValue:
    """Normalize an action name. Returns ActionName.UNKNOWN only for the literal 'unknown' sentinel."""
    if isinstance(value, ActionName):
        return value
    normalized = value.strip().lower()
    if not normalized or normalized == ActionName.UNKNOWN:
