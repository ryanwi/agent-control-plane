...
| `agent_control_plane` | `PolicyEngine`, `ProposalRouter`, `SessionRiskAccumulator`, `RuntimeMonitor`, `RuntimeMonitorConfig`, `CancellableExecution`, `ApprovalGate`, `BudgetTracker`, `ConcurrencyGuard`, `KillSwitch`, `PreconditionVerifier`, `EventStore`, `SessionManager`, `AgentRegistry`, `DelegationGuard`, `CrashRecovery`, `TimeoutEscalation`, `ModelRegistry`, `RiskClassifier`, `DefaultRiskClassifier`, `ConditionEvaluator`, `ParallelPolicyEvaluator` | Individual engines for direct wiring (advanced). `CancellableExecution` is a host-implemented protocol. |
